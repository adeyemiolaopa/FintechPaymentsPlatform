...
public sealed class IdentityOutboxPublisher : BackgroundService
{
    ...
    {
        if (!_options.PublisherEnabled) return;
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogWarning(exception, "Identity outbox publish cycle failed.");
            }
...
FOR UPDATE SKIP LOCKED").ToListAsync(cancellationToken).ConfigureAwait(false);
        ...
        foreach (var message in pending)
        {
            try
            {
                ...
                _logger.LogInformation("Published identity outbox message {OutboxMessageId} event {EventId} type {EventType} to {TopicPartitionOffset}", message.Id, message.EventId, message.EventType, result.TopicPartitionOffset);
            }
            catch (Exception exception) when (exception is not OperationCanceledException)
            {
                ...
                _logger.LogWarning(exception, "Failed to publish identity outbox message {OutboxMessageId} event {EventId} attempt {AttemptCount}", message.Id, message.EventId, message.AttemptCount);
            }
        }
    ...
    private async Task DeletePublishedMessagesAsync(IdentityDbContext dbContext, DateTimeOffset now, CancellationToken cancellationToken)
    {
...
DELETE FROM identity.outbox_messages
WHERE ""Id"" IN (
    SELECT ""Id""
    ...
    FOR UPDATE SKIP LOCKED
        ...
        if (deleted > 0) _logger.LogInformation("Deleted {DeletedCount} published identity outbox messages older than {Cutoff}", deleted, cutoff);
    }
