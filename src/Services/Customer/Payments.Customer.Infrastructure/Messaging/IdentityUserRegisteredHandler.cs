...
public sealed class IdentityUserRegisteredHandler
{
    ...
    public async Task HandleAsync(IntegrationEventEnvelope<IdentityUserRegisteredIntegrationEvent> envelope, CancellationToken cancellationToken = default)
    {
        ...
        if (existing is null)
        {
            ...
            _repository.AddAudit(CustomerAuditEvent.Create("CustomerRegistered", envelope.Payload.UserId, customer.Id, _clock.UtcNow, envelope.CorrelationId));
            _repository.AddLifecycleOutbox(customer, _clock.UtcNow, envelope.CorrelationId, envelope.CausationId);
            _logger.LogInformation("Created customer {CustomerId} from identity user {UserId}", customer.Id, envelope.Payload.UserId);
        }

        _repository.AddProcessedEvent(ProcessedIntegrationEvent.Create(envelope.EventId, envelope.EventType, _clock.UtcNow));
