...
using Payments.Ledger.Api.Endpoints;
using Payments.Ledger.Api.Middleware;
using Payments.Ledger.Application.Ledger;
using Payments.Ledger.Infrastructure;
...
try
{
    ...
    builder.Host.UseSerilog((context, services, configuration) => configuration.ReadFrom.Configuration(context.Configuration).ReadFrom.Services(services).Enrich.FromLogContext().Enrich.WithProperty("ServiceName", "ledger-api").WriteTo.Console());
    ...
    builder.Services.AddPermissionAuthorization("ledger.account.create", "ledger.account.read", "ledger.transaction.post", "ledger.transaction.read", "ledger.transaction.reverse", "ledger.integrity.read", "ledger.audit.read");
    ...
    if (app.Environment.IsDevelopment() || app.Environment.IsEnvironment("Testing"))
    {
        ...
        await LedgerSeedData.SeedAsync(dbContext);
    }

    app.UseMiddleware<CorrelationIdMiddleware>();
    app.UseMiddleware<SecurityHeadersMiddleware>();
    app.UseSerilogRequestLogging();
    app.UseMiddleware<LedgerExceptionMiddleware>();
    ...
    if (app.Environment.IsDevelopment())
    {
        app.MapOpenApi();
        app.UseSwagger();
        app.UseSwaggerUI(options => options.SwaggerEndpoint("/swagger/v1/swagger.json", "Ledger API v1"));
    }
    ...
    app.MapLedgerReconciliationEndpoints();

    app.Run();
}
catch (Exception exception)
{
    Log.Fatal(exception, "Ledger API terminated unexpectedly");
}
finally
{
    Log.CloseAndFlush();
