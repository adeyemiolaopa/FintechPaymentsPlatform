...
public sealed class LedgerOutboxPublisher : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            _logger.LogInformation("ledger outbox publisher is disabled.");
            return;
        }
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogWarning(exception, "ledger outbox publish cycle failed.");
            }
...
SELECT * FROM ledger.outbox_messages
...
FOR UPDATE SKIP LOCKED").ToListAsync(cancellationToken).ConfigureAwait(false);
        ...
        foreach (var message in pending)
        {
            try
            {
                ...
                _logger.LogInformation("Published ledger outbox message {OutboxMessageId} event {EventId} type {EventType} to {TopicPartitionOffset}", message.Id, message.EventId, message.EventType, result.TopicPartitionOffset);
            }
            catch (Exception exception) when (exception is not OperationCanceledException)
            {
                ...
                _logger.LogWarning(exception, "Failed to publish ledger outbox message {OutboxMessageId} event {EventId} attempt {AttemptCount}", message.Id, message.EventId, message.AttemptCount);
            }
        }
    ...
    private async Task DeletePublishedMessagesAsync(LedgerDbContext dbContext, DateTimeOffset now, CancellationToken cancellationToken)
    {
...
DELETE FROM ledger.outbox_messages
WHERE ""Id"" IN (
    SELECT ""Id""
    ...
    FOR UPDATE SKIP LOCKED
        ...
        if (deleted > 0) _logger.LogInformation("Deleted {DeletedCount} published ledger outbox messages older than {Cutoff}", deleted, cutoff);
    }
